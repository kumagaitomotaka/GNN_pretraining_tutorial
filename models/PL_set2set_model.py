...
class PL_Set2Setmodel(pl.LightningModule):
    def __init__(self, task, model_type, num_atom_features, num_bond_features, finetune_dim):
        super(PL_Set2Setmodel, self).__init__()
        ...
        self.lin0 = Linear(num_atom_features, dim)

        nn = Sequential(Linear(num_bond_features, 128), ReLU(), Linear(128, dim * dim))
        self.convs = torch.nn.ModuleList()
        for i in range(3):
            self.convs.append(NNConv(dim, dim, nn, aggr='mean'))
        self.gru = GRU(dim, dim)

        self.set2set = Set2Set(dim, processing_steps=3)
        self.lin1 = Linear(2 * dim, dim)
        self.lin2 = Linear(dim, 2)
        
        self.dropout = Dropout(0.25) #dropout層を追加した
        
        if self.finetune_dim != 0:
            print('Fintune model!')
            self.f_lin = Linear(dim, finetune_dim)

    def forward(self, data):
        h = F.relu(self.lin0(data.x))
        g = h.unsqueeze(0) #次元の追加

        for conv in self.convs:
            m = F.relu(conv(h, data.edge_index, data.edge_attr))
            h, g = self.gru(m.unsqueeze(0), g)
            h = h.squeeze(0)
            h = self.dropout(h)
